...
<?if($arResult["ITEMS"]):?>
	...
	<div class="top_wrapper items_wrapper <?=$templateName;?>_template" <?=$bBigBlock?>>
	...
	<?endif;?>
		...
		<?/*<script>console.info(<?=json_encode($arItem)?>);</script>*/?>
			...
			<div class="col-lg-<?=$col;?><?if($bBigBlock):?> col-md-8 col-sm-12 col-xs-12<?else:?> col-md-4 col-sm-6 col-xs-6<?endif;?> col-xxs-12 item item-parent item_block<?=($bBigBlock ? ' big' : '');?> <?=($arParams['SET_LINE_ELEMENT_COUNT'] ? 'custom-line' : '');?>">
				...
				<div class="catalog_item_wrapp catalog_item item_wrap main_item_wrapper<?=($bBigBlock ? ' big' : '');?> <?=($arItem["OFFERS"] ? 'has-sku' : '')?>" id="<?=$arItem["strMainID"]?>">
					<div <?=$arItem['PROPERTIES']['SNYAY']['VALUE']?>  class="inner_wrap <?=$arParams["TYPE_VIEW_BASKET_BTN"]?>">
						<?if($arParams['SHOW_GALLERY'] == 'Y' && $arItem['OFFERS']):?>
							<div class="js-item-gallery hidden"><?\Aspro\Functions\CAsproMaxItem::showSectionGallery($arItem);?></div>
						<?endif;?>
						<?if($bBigBlock):?>
							<?$bFonImg = false;?>
							...
							<a class="absolute-full-block lazy absolute-full-block_bg_center scale_block_animate darken-bg-animate <?=($bFonImg ? '' : 'contain');?>" href="<?=$arItem['DETAIL_PAGE_URL'];?>" data-src="<?=$imageSrc?>" data-bg="<?=$imageSrc?>" style="background-image:url(<?=\Aspro\Functions\CAsproMax::showBlankImg($imageSrc);?>)"><span></span></a>
						<?endif;?>
						<div class="image_wrapper_block<?=($arParams['SHOW_PROPS'] == 'Y' && $arItem['DISPLAY_PROPERTIES'] && !$bBigBlock ? ' with-props' : '');?>">
							...
							<?if(!$bBigBlock):?>
								<?if($arParams['SHOW_PROPS'] == 'Y' && $arItem['DISPLAY_PROPERTIES']):?>
									<div class="properties properties_absolute scrollbar">
								...
								<?if($arParams['SHOW_GALLERY'] == 'Y' && $arParams['SHOW_PROPS'] != 'Y'):?>
									<?if($bUseSkuProps && $arItem["OFFERS"]):?>
										<?\Aspro\Functions\CAsproMaxItem::showSectionGallery($arItem["OFFERS"][$arItem["OFFERS_SELECTED"]]);?>
									<?else:?>
										<?\Aspro\Functions\CAsproMaxItem::showSectionGallery($arItem);?>
									<?endif;?>
								<?else:?>
						...
						<div class="item_info">
							...
							<div class="item-title">
								<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="<?=(!$bBigBlock ? 'dark_link' : '');?> option-font-bold font_sm"><span><?=$elementName;?></span></a>
							</div>
                ...
                <?
                $res = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arItem['ID'], "sort", "asc", array("CODE" => "AKTSIYA_NA_SAYTAKH"));
                if ($ob = $res->GetNext())
                {
                  $VALUEDA = $ob['VALUE_ENUM'];
                }
                if ($VALUEDA=='Да') {
                  $res = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arItem['ID'], "sort", "asc", array("CODE" => "SPETSIALNAYA_TSENA_PO_AKTSII"));
                  if ($ob = $res->GetNext()) {
                    $VALUE = $ob['VALUE'];
                    $arParams['PRICE_OLD']=$VALUE;
                  }
                }
                ?>
						...
						<?if(!$bBigBlock):?>
							<div class="footer_button <?=($arItem["OFFERS"] && $arItem['OFFERS_PROP'] ? 'has_offer_prop' : '');?> inner_content js_offers__<?=$arItem['ID'];?>_<?=$arParams["FILTER_HIT_PROP"]?><?=($arParams["TYPE_VIEW_BASKET_BTN"] == "TYPE_2" ? ' n-btn' : '')?>">
								<?if($arParams["TYPE_VIEW_BASKET_BTN"] != "TYPE_2"):?>
									<?if($arParams["TYPE_VIEW_BASKET_BTN"] == "TYPE_3"):?>
										...
										<?if(!$arItem["OFFERS"]):?>
											<div class="counter_wrapp <?=($arItem["OFFERS"] && $arParams["TYPE_SKU"] == "TYPE_1" ? 'woffers' : '')?> clearfix rounded3">
												...
												<div id="<?=$arItemIDs["ALL_ITEM_IDS"]['BASKET_ACTIONS']; ?>" class="button_block <?=($arAddToBasketData["ACTION"] == "ORDER"  || !$arAddToBasketData["CAN_BUY"] || !$arAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_LIST"] || $arAddToBasketData["ACTION"] == "SUBSCRIBE" || $arAddToBasketData["ACTION"] == "MORE" ? "wide" : "");?>">
													<!--noindex-->
													<?/*=$arAddToBasketData["HTML"]*/?>
													<? /* **** для первой страницы меняем местами Снят с производства и кнопку Аналоги ***** */  ?>
<?
$my=$arAddToBasketData["HTML"];
$fi='<br>Снят с производства';
$pos=strpos($my, $fi);
...
<?
	...
	$sisya=str_ireplace('<br>Снят с производства','',$arAddToBasketData["HTML"]);
	echo "Снят с производства<br>".$sisya;

}
?>
													<? /* **** конец для первой страницы меняем местами Снят с производства и кнопку Аналоги ***** */  ?>
													<!--/noindex-->
												</div>
											</div>
											<?
									...
									<?if($arItem["OFFERS"])
									{
										if(!empty($arItem['OFFERS_PROP'])){?>
											<div class="bx_catalog_item_scu wrapper_sku" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PROP_DIV']; ?>" data-site_id="<?=SITE_ID;?>" data-id="<?=$arItem["ID"];?>" data-offer_id="<?=$arItem["OFFERS"][$arItem["OFFERS_SELECTED"]]["ID"];?>" data-propertyid="<?=$arItem["OFFERS"][$arItem["OFFERS_SELECTED"]]["PROPERTIES"]["CML2_LINK"]["ID"];?>" data-offer_iblockid="<?=$arItem["OFFERS"][$arItem["OFFERS_SELECTED"]]["IBLOCK_ID"];?>">
												<?$arSkuTemplate = array();?>
												<?$arSkuTemplate=CMax::GetSKUPropsArray($arItem['OFFERS_PROPS_JS'], $arResult["SKU_IBLOCK_ID"], $arParams["DISPLAY_TYPE"], $arParams["OFFER_HIDE_NAME_PROPS"], "N", $arItem, $arParams['OFFER_SHOW_PREVIEW_PICTURE_PROPS']);?>
						...
						<?else:?>
							<div class="footer_button <?=($arItem["OFFERS"] && $arItem['OFFERS_PROP'] ? 'has_offer_prop' : '');?> inner_content js_offers__<?=$arItem['ID'];?>_<?=$arParams["FILTER_HIT_PROP"]?><?=($arParams["TYPE_VIEW_BASKET_BTN"] == "TYPE_2" ? ' n-btn' : '')?> ce_cmp_visible">
								<div class="counter_wrapp clearfix offer_buy_block<?=(($arAddToBasketData["ACTION"] == "NOTHING") ? ' n-btn' : '');?> ce_cmp_visible">
									<div class="button_block">
										<!--noindex-->
											<?=$arAddToBasketData["HTML"]?>
