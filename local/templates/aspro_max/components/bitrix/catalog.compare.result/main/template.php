	...
	<div class="catalog-compare__inner loading">
		...
		<div class="table_compare catalog_block_template">
			<?if($arResult["SHOW_FIELDS"]):?>
				<div class="catalog-compare__items catalog_block items block flexbox flexbox--row owl-carousel owl-theme owl-bg-nav visible-nav" data-plugin-options='{"nav": true, "autoplay" : false, "dots": false, "autoplayTimeout" : "3000", "smartSpeed":500, "responsiveClass": true, "withSlide": "catalog-compare__props-slider", "rewind": true, "responsive":{"0":{"items": 1},"600":{"items": 2},"768":{"items": 3},"992":{"items": 4},"1200":{"items": 5}}}'>
					<?foreach($arResult["ITEMS"] as &$arElement){?>
            <?
            $res = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arElement['ID'], "sort", "asc", array("CODE" => "AKTSIYA_NA_SAYTAKH"));
            if ($ob = $res->GetNext())
            {
              $VALUEDA = $ob['VALUE_ENUM'];
            }
            if ($VALUEDA=='Да') {
              $res = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arElement['ID'], "sort", "asc", array("CODE" => "SPETSIALNAYA_TSENA_PO_AKTSII"));
              if ($ob = $res->GetNext()) {
                $VALUE = $ob['VALUE'];
			...
			<?if($arShowFileds || $arShowOfferFileds || $arShowProps || $arShowOfferProps):?>
				<div class="catalog-compare__props-slider owl-carousel owl-theme" data-plugin-options='{"nav": false, "dots": false, "autoplay" : false, "autoplayTimeout" : "3000", "smartSpeed":500, "responsiveClass": true, "withSlide1": "catalog-compare__items", "rewind": true, "responsive":{"0":{"items": 1},"600":{"items": 2},"768":{"items": 3},"992":{"items": 4},"1200":{"items": 5}}}'>
					<?foreach($arResult["ITEMS"] as $arElement){?>
