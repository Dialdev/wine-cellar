...
$db_props = CIBlockElement::GetList([],['IBLOCK_ID'=>$arElement['IBLOCK_ID'],'ID'=>$arElement['ID']],false,false,['ID','CODE','PRICE','IBLOCK_SECTION_ID','QUANTITY','PROPERTY_SNYAY','ACTIVE']);
if($r=$db_props->Fetch()){
	if($r['PROPERTY_SNYAY_VALUE']=='Уценен'||$r['ACTIVE']=='Y'){
		if($r['QUANTITY']<=0){
			$link=$APPLICATION->GetCurPage();
			$link = str_replace($r['CODE'].'/','',$link);
