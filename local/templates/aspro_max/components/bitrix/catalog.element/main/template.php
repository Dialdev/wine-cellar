...
<?
$res = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arResult['ID'], "sort", "asc", array("CODE" => "AKTSIYA_NA_SAYTAKH"));
if ($ob = $res->GetNext())
{
  $VALUEDA = $ob['VALUE_ENUM'];
}
if ($VALUEDA=='Да') {
  $res = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arResult['ID'], "sort", "asc", array("CODE" => "SPETSIALNAYA_TSENA_PO_AKTSII"));
  if ($ob = $res->GetNext()) {
    $VALUE = $ob['VALUE'];
...
if ($USER->IsAdmin()) {
	...
	//print_r($arResult['PROPERTIES']['rating']);
	//print_r($arParams["SHOW_RATING"]);
	//print_r($arResult['PROPERTIES']);
	//echo "</pre>";
}?>
