...
if($arResult["ITEMS"]){?>
	...
	<div id="custom-filter-outer-container">
	<div class="new-custom bx_filter bx_filter_vertical compact swipeignore">
								...
								switch ($arItem["DISPLAY_TYPE"]){
									...
									default://CHECKBOXES
										...
										<?foreach($arItem["VALUES"] as $val => $ar):?>
											<?
												if($ar['URL_ID']){
													$smartParts[0][0] = toLower($arItem['CODE']);
													// $smartParts[0][1] = urlencode(toLower($ar['URL_ID']));
													$smartParts[0][1] = toLower($ar['URL_ID']);
													$buffer_link = CBitrixCatalogSmartFilter::encodeSmartParts($smartParts);
													$buffer_link = str_replace("/clear/", "/".$buffer_link[0]."/", $arResult['SEF_DEL_FILTER_URL']);
												}
												$str = $buffer_link;
												preg_match_all('/[\p{Cyrillic} ]+/iu', $buffer_link, $match);
											...
											<label data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx_filter_param_label <?=($isSize ? "nab sku" : "");?> <?=($i==$count ? "last" : "");?> <? echo $ar["DISABLED"] ? 'disabled': '' ?>" for="<? echo $ar["CONTROL_ID"] ?>">
												<?if($buffer_link):?>
													<?
														$link =  Sotbit\Seometa\SeometaUrlTable::getByRealUrl($buffer_link);
														$buffer_link = $link['NEW_URL'] ? $link['NEW_URL'] : $buffer_link;
													?>
													<a href="<?=$arItem['NO_LINK'] == 'Y' ? 'javascript:void(0)': $buffer_link?>" class="filter-link custom-href"<?=$arItem['NO_LINK'] == 'Y' ? ' data-href="'.$buffer_link.'"':''?>>
												<?endif?>
												<span class="bx_filter_input_checkbox">
