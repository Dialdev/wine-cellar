...
<?if($arResult['ITEMS']):?>
	...
	?>
	<?if(!$arParams['IS_AJAX']):?>
		<div class="content_wrapper_block <?=$templateName;?> content_news2">
	...
	<?endif;?>
			<?
			$indexItem = 0;//echo '<pre>', var_dump($arResult['ITEMS']),'</pre>';
			$useImageAlternate = ($arParams['USE_BG_IMAGE_ALTERNATE']=="Y");
			
			//$alternateNumber = ((int)$arParams['SIZE_IN_ROW'])*2 - 2;
			...
			<?foreach($arResult['ITEMS'] as $i => $arItem):?>
				...
				//$bBgImage = ($arItem['PROPERTIES']['TYPE_BLOCK']['VALUE'] != '');
				if($useImageAlternate){
					/*$bBgImage = ($indexItem == 1 || $indexItem == $alternateNumber ? true : false);
						if($bBgImage){
				...
				//var_dump($arItem['PROPERTIES']['TYPE_BLOCK']['VALUE']);
				...
				$bHalfWrapper = false;
				//var_dump($arParams['ALL_BLOCK_BG']);
				...
				<?if($arParams['HALF_BLOCK'] != 'Y' || ($arParams['HALF_BLOCK'] == 'Y' && $arParams['IS_AJAX'] != 'Y' && $i < 2)):?>
					<div class="item-wrapper col-md-3<?//=$col;?> col-sm-6 col-xs-6 col-xxs-12 clearfix <?=$arItem['PROPERTIES']['TYPE_BLOCK']['VALUE_XML_ID'];?> <?=$dop_class;?>" data-ref="mixitup-target">
				<?endif;?>

					<?if($arParams['HALF_BLOCK'] == 'Y' && $arParams['IS_AJAX'] != 'Y' && $i == 1):?>
					...
					<?else:?>
					<div class="item<?=($bBordered ? ' bordered box-shadow rounded3' : '');?><?=(!$bImage ? ' no-img' : '');?><?=(($arResult['HAS_TITLE_FON'] == 'Y' || $useImageAlternate) ? ' long' : '');?> clearfix" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
						<?if($bImage):?>
					...
					<?endif;?>
						<div class="inner-text<?=($bShowSection ? ' with-section' : '');?><?=($bActiveDate ? ' with-date' : '');?><?=($arParams['TITLE_SHOW_FON'] == 'Y' ? ' with-fon' : '');?>">
							...
							<?// date active period?>
							<?if($bActiveDate):?>
								<div class="period-block<?=($bShowSection && !$bLineImg ? '' : ' muted');?> <?=($arParams['HALF_BLOCK'] == 'Y' && $i ? 'font_xxs' : 'font_xs');?>">
				...
				<?if($arParams['HALF_BLOCK'] != 'Y' || ($arParams['HALF_BLOCK'] == 'Y' && ($arParams['IS_AJAX'] == 'Y' || ($arParams['IS_AJAX'] != 'Y' && !$i)))):?>
				</div>
				<?endif;?>
				<?
