	...
	if($arItems)
	{
		foreach($arItems["ITEMS"] as $key => $arItem)
		{
			...
			if($arPost["PARAMS"]["SHOW_GALLERY"] == "Y")
			{
				...
				$arItems["ITEMS"][$key]["GALLERY_HTML"] = \Aspro\Functions\CAsproMaxItem::showSectionGallery($arItems["ITEMS"][$key], 'GALLERY', true);
			}
		...
		ChangeInfo = function()
		{
			...
			if(-1 < index)
			{
				...
				if(obOffers[index].GALLERY)
				{
					...
					/*if(wrapper.find('.js-item-gallery'))
					{
						if(wrapper.find('.image_wrapper_block .thumb .section-gallery-wrapper').length)
							wrapper.find('.image_wrapper_block .thumb .section-gallery-wrapper').append(wrapper.find('.js-item-gallery .thumb .section-gallery-wrapper').html().replace(/_active/, '').replace(/section-gallery-wrapper__item_hidden/, ''));
						else
							wrapper.find('.image_wrapper_block .thumb').append(wrapper.find('.js-item-gallery .thumb').html());
					}*/
					InitLazyLoad();
				}
				else
